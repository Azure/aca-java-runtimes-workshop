...
Select the owner of the repository, enter a name for your new repository (eg. `aca-java-runtimes-workshop`), set the repo visibility to `public` and click on `Create repository from template`.
...
[INFO]
[INFO] Azure Container Apps and Java Runtimes Workshop :: Micronaut 1.0.0-SNAPSHOT SUCCESS [  5.131 s]
[INFO] springboot-app 0.0.1-SNAPSHOT ...................... SUCCESS [  1.841 s]
[INFO] Azure Container Apps and Java Runtimes Workshop 1.0.0-SNAPSHOT SUCCESS [  0.102 s]
[INFO] Azure Container Apps and Java Runtimes Workshop :: Quarkus 1.0.0-SNAPSHOT SUCCESS [  5.528 s]
[INFO] ------------------------------------------------------------------------
[INFO] BUILD SUCCESS
[INFO] ------------------------------------------------------------------------
...
Once you have created your Azure subscription, we'll spend some time to create all the resources needed to deploy our Apps. We'll do that mostly using the Azure CLI, but at any point you can go to https://portal.azure.com[Azure Portal] to have a visual look at the resources we'll be configuring.
...
https://learn.microsoft.com/azure/azure-monitor/logs/quick-create-workspace?tabs=azure-portal[Log Analytics workspace] is the environment for Azure Monitor log data. Each workspace has its own data repository and configuration, and data sources and solutions are configured to store their data in a particular workspace. We will use the same workspace for most of the Azure resources we will be creating.
...
You can execute a SQL statement directly in the database with the following command to check the content of the `Statistics` table (which should be empty at this stage):
