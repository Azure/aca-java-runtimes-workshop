...
[WARNING]
====
If you are on Windows, you will need to either install the Windows Subsystem for Linux (WSL) and use a Linux distribution (eg. Ubuntu), or use DevContainers.
https://learn.microsoft.com/en-us/windows/wsl/install[Follow the instruction to install WSL] on Windows.
Here a snipped of a Windows script to setup the environment variables:

[source,shell]
----
include::{workshop-github-raw}/scripts/infra/azure.cmd[]
----
====
...
So, if you use DevContainer, make sure to set the `UNIQUE_IDENTIFIER` environment variable to a unique value (in lower case).
====
