...
Before executing it, you need to start the Postgres database.
When you package a Quarkus application with `mvn package` (JVM or native), Quarkus turns out to be using the production profile (`%prod`) and needs a Postgres to be up and running and configured.
For that, you first need to configure the `application.properties` file with the right Postgres URL.
...
Good.
Let's now create the same workflow for the Micronaut and SpringBoot applications before we do some performance tuning again.
...
You can execute it with the following command (make sure the Postgres database is running `docker compose -f postgres.yaml up`):
...
Make sure you have the Postgres database still up and running, and execute:
