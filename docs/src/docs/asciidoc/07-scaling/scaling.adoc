...
image::../images/dashboard-scale-setcpu.png[Screenshot of setting up CPU scaler]
...
image::../images/dashboard-scale-setmemory.png[Screenshot of setting up memory scaler]
...
Remember that we have a Postgres Database with three tables where we store our metrics.
You can execute the following SQL statements so you get all the metrics for Quarkus, Micronaut and SpringBoot.
