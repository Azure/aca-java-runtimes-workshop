...
To make sure you can follow this workshop, check if the region you are in supports Azure Container Apps by executing the following commandaz provider show -n Microsoft.App --query "resourceTypes[?resourceType=='managedEnvironments'].locations"
...
We will create three container apps, one for each of our Java runtimes (Quarkus, Micronaut and SpringBoot).
