...
* Setting up your development environment, either by installing all the needed tools manually, or installing a few tools and relying on VSCode and Dev Containers (easier as everything is setup for you).
...
- Install the *Windows Subsystem for Linux* (WSL) with a Linux distribution (eg. Ubuntu). You can follow the instructions https://learn.microsoft.com/en-us/windows/wsl/install[here].
- Use *Visual Studio Code* with the Dev Containers setup.
 on Windows.
====
...
[source,shell]
----
include::{workshop-github-raw}/scripts/infra/azure.sh[tag=adocSetupAzCli, indent=0]
----
...
So, if you use Dev Containers, make sure to set the `UNIQUE_IDENTIFIER` environment variable to a unique value (in lower case).
====
