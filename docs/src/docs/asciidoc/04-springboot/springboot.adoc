...
This is the root of the SpringBoot microservice source code that we will be using in this section.
...
Before creating the REST resource, we need to add some code to the existing main SpringBoot class that will bootstrap the application.
Add the following code to the `SpringbootApplication` under the `io/containerapps/javaruntime/workshop/springboot` package.

[[springboot-listing-rest-resource]]
.Bootstrapping SpringBoot Class
[source,indent=0]
----
...
[NOTE]
====
Notice that you need the `registerHints` method for native GraalVM image build (that we will do later).
This method is used to register the classes that will be used by the application at runtime.
We are also using an older PostgreSQL release as there is a bug in the latest version that prevents it to work.
====
...
Create the `SpringbootApplicationTests` class under the `src/test/java/io/containerapps/javaruntime/workshop/springboot` package.
...
Time to containerize them and deploy them to the Azure Container Apps that we have previsouly created.
